...
/**
 *
 * @type {import('../types/index').NeuJS}
 */
export default class NeuJS extends EventEmitter {
  ...
  /**
   *
   * @param {import('../types/index').NeuJSOptions} [options]
   */
  constructor (options = {}) {
  ...
  async #transmit (request, response) {
    if (u.ifStatic(request.path)) {
      ...
      return nextFunction(request, response, this.#serveFile)
    } else {
   ...
   */
  init (serverFn) {
    const server = serverFn(NeuJS.serverOptions, this.loadListener())
  ...
  static createApp (options) {
    return options ? new NeuJS(options) : new NeuJS()
  }

  static createRouter (options) {
    return options ? new NeuRouter(options) : new NeuRouter()
  }
  ...
  static async load () {
    ...
    if (neuSpec.ext === '.json') {
      ...
      const neujsApp = config.env ? new NeuJS(config.env) : new NeuJS()
