...
function getStaticFiles (path) {
  ...
  while (++index < length) {
    ...
    if (statSync(filePath).isDirectory()) {
      const subfiles = getStaticFiles(filePath)
      response = { ...response, ...subfiles }
    } else {
      const file = filePath.split('/public')[1]
      console.log(file)
...
export default class Neumatter extends EventEmitter {
  ...
  constructor (options = {}) {
    ...
    this.staticMap = options.static ? getStaticFiles(options.static) : {}
  ...
  async #transmit (request, response) {
    if (this.staticMap[request.path]) {
  ...
  static async load () {
    ...
    const neuSpec = root.files.find({ fullname: 'neumatter.config.json' })[0]
    if (!neuSpec) throw new NeuError('No config file found.')
    // get configuration options
    const config = await u.readJSON(neuSpec.path)
    // init new Application
    const application = config.env ? new Neumatter(config.env) : new Neumatter()
    ...
    await application.useMany(routers)
    console.log(application.router.routeMap.get('/orders').methods.GET)
    return application
  }
