...
import ContentTypeDB from './content-type-db.js'

const contentTypeDB = await ContentTypeDB.connect()
...
export default class MIME {
  ...
  static typify (path) {
    const header = contentTypeDB.fromExtension(path.slice(path.lastIndexOf('.') + 1))
    if (header) return header.mime
