...
export default class NeuResponse extends ServerResponse {
  ...
  get (input) {
    if (!input) return this.getHeaders()
    if (IS.array(input)) {
      const headers = this.getHeaders()
      const result = {}
      NeuPack.each(input, key => {
        key = u.toFirstUpperCase(key)
        result[key] = headers[key] || null
      })
      return result
    }
    if (IS.string(input)) {
      return this.getHeader(u.toFirstUpperCase(input))
    }
    throw NeuError('NeuResponse.get: Could not recognize type of input. Accepts: [undefined, Array<strings>, string]')
  }
  ...
  set (input, value) {
    ...
    if (IS.array(input)) {
      NeuPack.each(input, header => {
        if (!IS.object(header)) throw NeuTypeError(header, 'object')
        this.set(header)
      })
      return this
    }
  ...
  redirect (...args) {
    const address = args.length === 1 ? args[0] : args.filter(el => IS.string(el))[0]
    const status = args.length === 1 ? 302 : args.filter(el => IS.number(el))[0]
    this.statusCode = status
    this.writeHead(status, { Location: address })
    this.end()
  }

  send (message) {
    let chunk = message
    if (!this.hasType) this.setHeader(MIME.CONTENT_TYPE, MIME.HEADERS.JSON)
    if (!this.statusCode) this.status(200)
    if (!this.hasContentLength) {
      // this.setHeader(HEADERS.CONTENT_LENGTH, Buffer.byteLength(message))
      const encoding = typeof chunk === 'string' ? 'utf8' : undefined
      if (chunk !== undefined) {
